...
@Version     :0.1
@Description : Implementation of SSA-GAN for CUB Dataset
'''
from model import NetG, NetD
from DAMSM import LLM_ENCODER
import os
import sys
import time
import random
...
def parse_args():
    parser = argparse.ArgumentParser(description='Playing with the well-trained model on CUB dataset')
    parser.add_argument('--json', dest='json_file',
                        help='dataset json file',
                        default='./dataset_cub.json', type=str)  # Use CUB JSON file here
    parser.add_argument('--use_gpu', dest='use_gpu', type=bool, default=False)
    parser.add_argument('--model_path', type=str,
                        default='/content/drive/MyDrive/GRAD/text2image/tmp/bird_sloss01/64/models/netG_156.pth' % root)  # Update path for CUB model
    parser.add_argument('--rnn_encoder', type=str,
                        default='%s/tmp/cub/64/models/text_encoder_120.pth' % root)  # Update path for CUB RNN encoder
...
def main(args):
    ...
    # load rnn encoder
    text_encoder = LLM_ENCODER(dataset_json['n_words'], nhidden=dataset_json['text_embed_dim'])
    ...
    with torch.no_grad():
        ...
        cap_number = 10000
        main_path = 'result/mani/cap_%s_0_cub_ch64' % (str(cap_number))
