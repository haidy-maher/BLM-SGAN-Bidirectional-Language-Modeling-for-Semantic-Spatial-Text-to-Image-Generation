...
from DAMSM import LLM_ENCODER, CNN_ENCODER
...
def write_images_losses(writer, imgs, fake_imgs, errD, d_loss, errG, DAMSM, epoch):
    index = epoch
    # Print the loss values and epoch
     # Prepare log information
    log_info = (
        f"Epoch {epoch}:\n"
        f"Discriminator Loss (errD): {errD}\n"
        f"Discriminator Gradient Penalty (MAGP): {d_loss}\n"
        f"Generator Loss (errG): {errG}\n"
        f"DAMSM Loss: {DAMSM}\n"
        "--------------------------------------------\n"
    )
    ...
    with open("writelogs.txt", "a") as log_file:  # 'a' for append mode
    ...
    for epoch in tqdm(range(state_epoch + 1, cfg.TRAIN.MAX_EPOCH + 1)):
        ...
        for step in tqdm(range(len(data_iter))):
            ...
            imags, captions, cap_lens, class_ids, keys = prepare_data(data)
                ...
                # words_embs: batch_size x nef x seq_len
            # sent_emb: batch_size x nef
            # Before concatenation in your training function
            ...
            # print(f"Shape of real_features: {real_features.shape}")
            # print(f"Shape of sent_emb: {sent_emb.shape}")
            # print(f"Shape of words_embs: {words_embs.shape}")
        ...
        if (epoch % cfg.TRAIN.GSAVE_INTERVAL == 0):
            ...
            print(f"Checkpoint #G_{epoch:03d} is saved successfully.") # save all in a text file for the tensorboard
        if (epoch % cfg.TRAIN.DSAVE_INTERVAL == 0):
            torch.save(netD.state_dict(), '%s/models/netD_%03d.pth' % (base_dir, epoch))
            print(f"Checkpoint #D_{epoch:03d} is saved successfully.")
...
if __name__ == "__main__":
    ...
    text_encoder = LLM_ENCODER(model_name='bert-base-uncased')    
    text_encoder.cuda()
    text_encoder.eval()
